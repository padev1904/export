...
LAST12_START = eomonth(CURRENT_DATE, -12) + timedelta(days=1)
...
DIMENSIONS = {
    'channel': {
        'select_label': 'd.TDistributionChannel',
        'alias': 'Canal',
        'joins': ['JOIN D_DistributionChannel d ON f.NIDDistributionChannel = d.NIDDistributionChannel'],
    },
    'region': {
        'select_label': 'd.TRegion',
        'alias': 'Regiao',
        'joins': ['JOIN D_Region d ON f.NIDRegion = d.NIDRegion'],
    },
    'country': {
        'select_label': 'd.TCountry',
        'alias': 'Pais',
        'joins': ['JOIN D_Country d ON f.NIDCountry = d.NIDCountry'],
    },
    'brand': {
        'select_label': 'd.TProductBrand',
        'alias': 'Marca',
        'joins': [
    ...
    'material_type': {
        'select_label': 'd.TMaterialType',
        'alias': 'TipoMaterial',
        'joins': [
            'JOIN D_Product p ON f.NIDProduct = p.NIDProduct',
            'JOIN D_MaterialType d ON p.NIDMaterialType = d.NIDMaterialType'
        ],
    },
}
...
def classify_question(question: str) -> SemanticSpec:
    ...
    raise ValueError(f'Pergunta nao suportada pelo gerador revC: {question}')
...
SELECT
    Mes,
    {f"{DIMENSIONS[spec.dimension]['alias']}, " if spec.dimension else ""}
...
WITH monthly_sales AS (
    SELECT
        f.MonthStart AS Mes,
        {dim_select}
        SUM({measure['expr']}) AS {measure['alias']}
    FROM F_Invoice f
    {' '.join(joins)}
    WHERE {' AND '.join(filters)}
      AND f.BillingDocumentDate >= {yyyymmdd(LAST6_START)}
...
SELECT
    ...
    CASE
        ...
        ELSE (({measure['alias']} - LAG({measure['alias']}) OVER ({part}ORDER BY Mes)) * 100.0) /
             NULLIF(LAG({measure['alias']}) OVER ({part}ORDER BY Mes), 0)
...
SELECT
    ...
    SUM(CASE
        WHEN f.BillingDocumentDate >= {yyyymmdd(CUR_MONTH_START)} AND f.BillingDocumentDate < {yyyymmdd(NEXT_MONTH_START)}
        THEN {measure['expr']} ELSE 0 END) AS ValorMesAtual,
    SUM(CASE
        WHEN f.BillingDocumentDate >= {yyyymmdd(SAME_MONTH_LAST_YEAR_START)} AND f.BillingDocumentDate < {yyyymmdd(SAME_MONTH_LAST_YEAR_NEXT)}
        THEN {measure['expr']} ELSE 0 END) AS ValorMesmoMesAnoAnterior,
    SUM(CASE
        WHEN f.BillingDocumentDate >= {yyyymmdd(SAME_MONTH_LAST_YEAR_START)} AND f.BillingDocumentDate < {yyyymmdd(SAME_MONTH_LAST_YEAR_NEXT)}
        THEN {measure['expr']} ELSE 0 END) AS BaseComparacao,
    CASE
        WHEN SUM(CASE
            WHEN f.BillingDocumentDate >= {yyyymmdd(SAME_MONTH_LAST_YEAR_START)} AND f.BillingDocumentDate < {yyyymmdd(SAME_MONTH_LAST_YEAR_NEXT)}
            THEN {measure['expr']} ELSE 0 END) = 0 THEN NULL
        ELSE ((SUM(CASE
            WHEN f.BillingDocumentDate >= {yyyymmdd(CUR_MONTH_START)} AND f.BillingDocumentDate < {yyyymmdd(NEXT_MONTH_START)}
            THEN {measure['expr']} ELSE 0 END) -
            SUM(CASE
            WHEN f.BillingDocumentDate >= {yyyymmdd(SAME_MONTH_LAST_YEAR_START)} AND f.BillingDocumentDate < {yyyymmdd(SAME_MONTH_LAST_YEAR_NEXT)}
            THEN {measure['expr']} ELSE 0 END)) * 100.0) /
            NULLIF(SUM(CASE
            WHEN f.BillingDocumentDate >= {yyyymmdd(SAME_MONTH_LAST_YEAR_START)} AND f.BillingDocumentDate < {yyyymmdd(SAME_MONTH_LAST_YEAR_NEXT)}
            THEN {measure['expr']} ELSE 0 END), 0)
    END AS VariacaoPercentual
FROM F_Invoice f
{' '.join(joins)}
WHERE {' AND '.join(filters)}
...
def load_json_table(path: Path) -> pd.DataFrame:
    text = path.read_text(encoding='utf-16').replace('\r', '').replace('\n', '')
    return pd.DataFrame(json.loads(text))

def build_sqlite_db(zip_root: Path, sqlite_path: Path) -> sqlite3.Connection:
    fi = pd.read_csv(zip_root / 'documentation' / 'f_invoice_sample.csv', sep=';')
    fi['BillingDocumentDate'] = fi['BillingDocumentDate'].astype(int)
    fi['BillingDocumentIsCancelled'] = fi['BillingDocumentIsCancelled'].astype(int)
    fi['IsItAnAdditionalCalculatedRecord'] = fi['IsItAnAdditionalCalculatedRecord'].astype(int)
    fi['BillingYear'] = fi['BillingDocumentDate'] // 10000
    fi['BillingMonth'] = (fi['BillingDocumentDate'] // 100) % 100
    fi['BillingYearMonth'] = fi['BillingDocumentDate'] // 100
    fi['MonthStart'] = pd.to_datetime(fi['BillingYearMonth'].astype(str) + '01', format='%Y%m%d').dt.strftime('%Y-%m-01')

    if sqlite_path.exists():
        sqlite_path.unlink()
    conn = sqlite3.connect(sqlite_path)
    fi.to_sql('F_Invoice', conn, index=False)

    for name in ['D_DistributionChannel.json','D_Region.json','D_Country.json','D_Product.json','D_ProductBrand.json','D_ProductFamily.json','D_MaterialType.json']:
        df = load_json_table(zip_root / 'documentation' / name)
        df.to_sql(name[:-5], conn, index=False)
    return conn
