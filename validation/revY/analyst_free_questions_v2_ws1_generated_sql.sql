...
SELECT
    ...
    (ValorLiquidoFaturado * 100.0) / NULLIF(SUM(ValorLiquidoFaturado) OVER (PARTITION BY Mes, CanalDistribuicao), 0) AS Percentagem
FROM base
ORDER BY Mes ASC, CanalDistribuicao ASC, Percentagem DESC, OrganizacaoVendas ASC;
...
SELECT
    ...
    MarcaProduto,
    MargemBruta,
    (MargemBruta * 100.0) / NULLIF(SUM(MargemBruta) OVER (PARTITION BY Mes, FamiliaProduto), 0) AS Percentagem
FROM base
ORDER BY Mes ASC, FamiliaProduto ASC, Percentagem DESC, MarcaProduto ASC;
